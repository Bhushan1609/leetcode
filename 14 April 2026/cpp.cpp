...
ll recursion(int i,int j,vd<ll>&positions,vd<int>&robot,vdd<ll>&dp){
    if(i>=len(robot))
        return 0;
    if(j>=len(positions))
        return 1E12;
    if(dp[i][j]!=-1)
        return dp[i][j];
    return dp[i][j]=min(abs(positions[j]-robot[i])+1LL*recursion(i+1,j+1,positions,robot,dp),recursion(i,j+1,positions,robot,dp));
}
...
public:
    long long minimumTotalDistance(vector<int>& robot, vector<vector<int>>& factory) {
        ...
        FOR(i,len(factory))
            FOR(factory[i][1])
                positions.eb(factory[i][0]);
        vdd<ll>dp(len(robot)+1,vd<ll>(len(positions)+1,-1));
        return recursion(0,0,positions,robot,dp);
    }
};
...
void solve() {
    Solution Obj;
    vd<int>input;
    return print(Obj.function_name(input));
}
int main() {
    #ifndef ONLINE_JUDGE
