...
int recursion(int i,int f1,int f2,string &word){
    if(i>=word.size())
        return 0;
    if(dp[i][f1][f2] != -1)
        return dp[i][f1][f2] ;
    ...
    int x2=f2/6;
    int y2=f2%6;
    //No finger Used so far
    if(f1==26 && f2==26){
        int finger1=0+recursion(i+1,word[i]-'A',f2,word);
        int finger2=0+recursion(i+1,f1,word[i]-'A',word);
        return dp[i][f1][f2]=min(finger1,finger2);
    }
    //Second finger used
    else if(f1==26){
        int finger1=0+recursion(i+1,word[i]-'A',f2,word);
        int finger2=abs(x2-x)+abs(y2-y)+recursion(i+1,f1,word[i]-'A',word);
        return dp[i][f1][f2]=min(finger1,finger2);
    }else if(f2==26){
        int finger1=abs(x1-x)+abs(y1-y)+recursion(i+1,word[i]-'A',f2,word);
        int finger2=0+recursion(i+1,f1,word[i]-'A',word);
        return dp[i][f1][f2]=min(finger1,finger2);
    }
    int finger1=abs(x1-x)+abs(y1-y)+recursion(i+1,word[i]-'A',f2,word);
...
void solve() {
    Solution Obj;
    string word="CAKE";
    return print(Obj.minimumDistance(word));
}
int main() {
    #ifndef ONLINE_JUDGE
