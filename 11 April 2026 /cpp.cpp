...
public:
    int minimumDistance(vector<int>& nums) {
        int n=len(nums),ans=1E6;
        unordered_map<int,int>next_first_occurence;
        vd<int>next_first_occurence_index(n,-1);
        FOR_R(i,n){
            if(next_first_occurence.count(nums[i]))
                next_first_occurence_index[i]=next_first_occurence[nums[i]];
            next_first_occurence[nums[i]]=(int)i;
        }
        FOR(i,n){
            int firstIndex=(int)i;
            int secondIndex=(int)next_first_occurence_index[i];
            if(secondIndex!=-1){
                int thirdIndex=next_first_occurence_index[secondIndex];
                if(thirdIndex!=-1)
                    ans=min(ans,thirdIndex-firstIndex);
            }
        }
...
void solve() {
    Solution Obj;
    vd<int>input={1,2,1,1,3};
    return print(Obj.minimumDistance(input));
}
int main() {
    #ifndef ONLINE_JUDGE
